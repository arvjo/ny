...
NumberGame.prototype.init = function (options) {
	...
	this.amount = 4;//GLOBAL.NUMBER_RANGE[options.range];
	this.min = 1;
	this.max = 4;
...
NumberGame.prototype.tryNumber = function (number, offset) {
	...
	DraggableObject.prototype.setTry.call(this, this.lastTry);
	this._currentTries++;
	this._totalTries++;
	if (!this.lastTry) {
...
/*
NumberGame.prototype._getTry = function(){
	return this.lastTry;
};*/
...
NumberGame.prototype.updateObjects = function(){
	var range = this._getRange();
	//alert('hej1: ' + this.goalObject);
	if(this.dragObject ){
		range = this._getRange();
		//alert('curr: ' +this.currentNumber);
...
/*Set relative false*/
NumberGame.prototype.setRelativeTrue = function (){
	this.isRelative = true;
};
